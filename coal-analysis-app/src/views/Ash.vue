<template>
  <div class="page ash-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">灰分测定</h2>
        <p class="page-subtitle">依据 GB/T 212-2008 — 缓慢灰化法</p>
      </div>
    ...
    <div class="average-section" v-if="average">
      <div class="avg-card">
        <div class="avg-label">两组平均灰分质量分数</div>
...
import { calcAsh, calcAverage } from '@/utils/calculation'
...
const timerPresets = [
  { seconds: 1800, label: '500°C 保持 30min' },
  { seconds: 3600, label: '815°C 保持 1h' },
  { seconds: 1200, label: '检查性灼烧 20min' },
  { seconds: 2400, label: '快速灰化 40min' },
]
...
const r1 = computed(() => calcAsh(group1.m1, group1.m2, toDryWeights(group1)))
const r2 = computed(() => calcAsh(group2.m1, group2.m2, toDryWeights(group2)))
...
const result1Items = computed(() => [
  ...
  { label: 'm₄ (剩余质量)', value: formatDecimals(r1.value.m4, 5) },
...
const result2Items = computed(() => [
  ...
  { label: 'm₄ (剩余质量)', value: formatDecimals(r2.value.m4, 5) },
...
.avg-card {
  background: linear-gradient(135deg, #e37400, #c46200);
  color: #fff;
  border-radius: 12px;
  padding: 20px;
  text-align: center;
...
.ash-page :deep(.input-group) { min-width: 0; }
</style>
